...
function transformHospital(raw: { id: string | number; name: string }): Hospital {
  return { id: raw.id, name: raw.name }
}

function transformServiceType(raw: { id: string | number; name: string }): ServiceType {
  return { id: raw.id, name: raw.name }
}
...
export const authApi = {
  ...
  logout: () => apiFetch<{ message: string }>('/user/logout', { method: 'POST' }),
}
